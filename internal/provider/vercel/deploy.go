...
func (p *VercelProvider) TriggerDeploy(ctx context.Context, account *domain.ProviderAccount, binding *domain.Binding, slot *domain.Slot) (*domain.DeployEvent, error) {
	cl := provider.NewClient(p.baseURL)

	latestID, name, err := p.latestDeployment(ctx, cl, account, binding)
	if err != nil {
		return nil, err
	}
	...
	path := p.buildPath("/v13/deployments", account)
...
func (p *VercelProvider) latestDeployment(ctx context.Context, cl *provider.Client, account *domain.ProviderAccount, binding *domain.Binding) (string, string, error) {
	path := p.buildPath(fmt.Sprintf("/v6/deployments?projectId=%s&limit=1", url.QueryEscape(binding.ExternalID)), account)
...
func (p *VercelProvider) ListDeployments(ctx context.Context, account *domain.ProviderAccount, binding *domain.Binding) ([]domain.DeployEvent, error) {
	cl := provider.NewClient(p.baseURL)
	path := p.buildPath(fmt.Sprintf("/v6/deployments?projectId=%s&limit=20", url.QueryEscape(binding.ExternalID)), account)
...
func (p *VercelProvider) GetBuildLogs(ctx context.Context, account *domain.ProviderAccount, binding *domain.Binding, deployID string, tail int) (domain.LogChunk, error) {
	cl := provider.NewClient(p.baseURL)
	path := p.buildPath("/v3/deployments/"+url.PathEscape(deployID)+"/events", account)
